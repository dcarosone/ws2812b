...
const APP: () = {
    ...
    fn init() -> init::LateResources {
        ...
        let timer1 = Timer::tim1(device.TIM1, LATCH_DELAY, clocks, &mut rcc.apb1);
        let timer3 = Timer::tim3(device.TIM3, LOG_FREQUENCY, clocks, &mut rcc.apb1);
    ...
    // IDLE LOOP
    #[idle(resources = [SLEEP_CYCLES,DWT])]
    fn idle(t: &mut Threshold, mut r: idle::Resources) -> ! {
        loop {
            rtfm::atomic(t, |t| {
                let dwt = DWT.borrow(t);
                let sleep_cycles = SLEEP_CYCLES.borrow_mut(t);
